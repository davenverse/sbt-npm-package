...
object PackageFile {
  ...
  def writePackageJson(
    ...
    if (Files.exists(packageJsonFile.toPath()))
      Files.delete(packageJsonFile.toPath())
    else ()
    write(
      streams.log,
      packageJsonFile,
  ...
  private def json(
    ...
    val npmManifestDependencies =
      NpmDependencies.collectFromClasspath(fullClasspath)
    val dependencies =
      npmDependencies ++ (
        if (currentConfiguration == Compile)
          npmManifestDependencies.compileDependencies
        else npmManifestDependencies.testDependencies
      )
    val devDependencies =
      npmDevDependencies ++ (
        if (currentConfiguration == Compile)
          npmManifestDependencies.compileDevDependencies
    ...
    val packageJson =
      Json
        .obj(additionalNpmConfig.toSeq: _*)
        .deepMerge(
          Json
            .obj(
              "name" -> name.asJson,
              ...
              "dependencies" -> Json.obj(
                resolveDependencies(dependencies, npmResolutions, log).map {
                  case (a, a2) => (a, a2.asJson)
                }: _*
              ),
              "devDependencies" -> Json.obj(
                resolveDependencies(devDependencies, npmResolutions, log).map {
                  case (a, a2) => (a, a2.asJson)
                }: _*
              ),
  ...
  /** Resolves multiple occurrences of a dependency to a same package.
    ...
    *   - If they refer to different versions, pick the one defined in
    *     `resolutions` (or fail if there is no such resolution).
    *
    * @return
    ...
    * @param resolutions
    *   The resolutions to use in case of conflict (they will be ignored if
  ...
  private def resolveDependencies(
    ...
    val unusedResolutions =
      resolutions.filter { case (name, resolution) =>
        resolvedDependencies.exists { case (n, v) =>
          n == name && v != resolution
        }
      }
